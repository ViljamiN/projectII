...
const addQuestion = async ({ params, request, response, render, state }) => {
  ...
  if (passes) {
    await dataBaseService.createQuestion(id, topicID, questionData.text);
    response.redirect(`/topics/${topicID}`);
  } else {
    questionData.errors = errors;
    questionData.questions = await dataBaseService.getQuestionsByTopicId(
      topicID
    );
    render("partials/questionsView.eta", questionData);
  }
};
